  ...
  async run(message: CommandoMessage) {
    if (cooldown == true) {
      const embed = new MessageEmbed({
        ...
        description:
          "> **Please wait " +
          moment
            .duration(timestamp + 30000 - Date.now(), "milliseconds")
            .humanize() +
    ...
    try {
      ...
      const res = await axios.get<any>(
        "https://www.marinetraffic.com/vesselDetails/latestPosition/shipid:3521181",
        {
          headers: {
            "User-Agent":
    ...
    try {
      axios({
        ...
        embed
          .attachFiles([new MessageAttachment(response.data, "map.png")])
